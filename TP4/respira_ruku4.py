...
def ruku4(f, x0, t0, tf, h):
    ...
    time_step = t0
    while time_step <= tf:
        _ruku4_global_tArr.append(time_step)
        time_step += step

    global _ruku4_global_x
    _ruku4_global_x = np.zeros((len(_ruku4_global_tArr), np.shape(x0)[0]))

    _ruku4_global_x[0, :] = x0

    stepOver2 = step / 2
    for k in range(1, len(_ruku4_global_tArr)):
        f1 = f(_ruku4_global_tArr[k - 1],
                _ruku4_global_x[k-1])
        
        f2 = f(_ruku4_global_tArr[k - 1] + stepOver2,
                _ruku4_global_x[k-1] + stepOver2 * f1)
        
        f3 = f(_ruku4_global_tArr[k - 1] + stepOver2,
                _ruku4_global_x[k-1] + stepOver2 * f2)
        
        f4 = f(_ruku4_global_tArr[k - 1] + step,
                _ruku4_global_x[k-1] + step * f3)
...
def mackeyglass_dxdt(t, x):
    ...
    if tMinus02 > 0:
        closest_time = closest(_ruku4_global_tArr, tMinus02)
        for index, element in enumerate(_ruku4_global_tArr):
